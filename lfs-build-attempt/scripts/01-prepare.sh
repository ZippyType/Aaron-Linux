#!/bin/bash
# Aaron Linux - Step 1: Prepare Environment
# Sets up directories, downloads sources
...
export LFS TARCH=x86_64

mkdir -p "$LOG_DIR"

log() {
    echo -e "\033[1;34m[$(date '+%Y-%m-%d %H:%M:%S')]\033[0m $1" | tee -a "$LOG_DIR/01-prepare.log"
}

log "Starting Aaron Linux build..."
...
mkdir -pv "$LFS"{,/sources,/tools,/scripts}

# Check disk space
DISK_SPACE=$(df -h "$LFS" | tail -1 | awk '{print $4}')
log "Available disk space: $DISK_SPACE"

# Download LFS 12.3 package list
log "Setting up package sources..."

# Create downloads script
cat > "$AARON_ROOT/scripts/packages/download.sh" << 'DLSCRIPT'
#!/bin/bash
...
chmod +x "$AARON_ROOT/scripts/packages/download.sh"

log "Created download script at: $AARON_ROOT/scripts/packages/download.sh"

echo ""
read -p "Run download script now? (y/n): " -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    log "Running download script..."
    bash "$AARON_ROOT/scripts/packages/download.sh"
else
    log "Download script skipped. Run manually when ready."
fi
